...
from morl.core.model_base import ModelBase
from morl.utils import machine_info
...
class Model(nn.Layer, ModelBase):
    """
    | `alias`: ``morl.Model``
    | `alias`: ``morl.core.paddle.agent.Model``

    | ``Model`` is a base class of morl for the neural network. 
    ...
    def sync_weights_to(self, target_model, decay=0.0):
        ...
        Args:
            target_model (`morl.Model`): an instance of ``Model`` that has 
