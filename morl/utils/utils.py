...
try:
    ...
    if paddle_version < 200 and paddle_version != 0:
        assert paddle_version >= 185, "morl requires paddle >= 1.8.5 and paddle < 2.0.0"
        _HAS_FLUID = True
    else:
        logger.warning(
            "paddlepaddle version: {}. The dynamic graph version of morl is under development, not fully tested and supported"
...
def check_version_for_xpu():
    ...
    """
    err = "To use xpu, morl requires paddle version >= 2.0.0. " \
...
def check_version_for_fluid():
    ...
    """
    err = "To use fluid version of examples, morl requires paddle version < 2.0.0. " \
          "Please make sure the version is consistent with your code."
    import paddle
    from paddle import fluid
