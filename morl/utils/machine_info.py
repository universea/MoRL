...
import socket
import subprocess
from morl.utils import logger, _HAS_FLUID, _HAS_PADDLE, _IS_WINDOWS
...
def is_gpu_available():
    """ check whether morl can access a GPU
    ...
    if _HAS_FLUID:
        ...
        if ret is True and not fluid.is_compiled_with_cuda():
            logger.warning("Found non-empty CUDA_VISIBLE_DEVICES. \
                But morl found that Paddle was not complied with CUDA, which may cause issues. \
                Thus morl will not use GPU.")
            return False
    if _HAS_PADDLE:
        import paddle
